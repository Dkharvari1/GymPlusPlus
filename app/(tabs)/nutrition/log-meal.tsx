...
export default function LogMeal() {
  ...
  /* 🔥 new param: `prefill` (optional) */
  const { day, prefill } = useLocalSearchParams<{ day?: string; prefill?: string }>();
  const dateId = day ?? format(new Date(), 'yyyy-MM-dd');

  const { favs, toggle } = useFavorites(uid);

  const [foods, setFoods] = useState<Food[]>([]);
  const [q, setQ] = useState(prefill ?? '');      // ← initialise search with any pre-fill
  ...
  const submit = async () => {
    ...
    await updateDoc(ref, {
      meals: [
        ...(snap.exists() && Array.isArray(snap.data().meals)
          ? snap.data().meals
          : []),
        {
          foodId: sel.id,
  ...
  return (
    <SafeAreaView style={st.safe}>
      ...
      <Pressable
        style={st.recBtn}
        onPress={() => router.push('/nutrition/recommend-food')}
      >
        <MaterialCommunityIcons name="lightbulb-on" size={18} color="#fff" />
        <Text style={st.recTxt}>Need ideas? See recommendations</Text>
      </Pressable>
        ...
        <View style={st.card}>
          ...
          <View style={st.pillBar}>
            {MEAL_TYPES.map((t) => (
              ...
              >
                <Text
                  style={[st.pillTxt, mealType === t && st.pillTxtActive]}
                >
...
const st = StyleSheet.create({
  ...
  recTxt: { color: '#fff', fontWeight: '600', marginLeft: 6, fontSize: 14 },
