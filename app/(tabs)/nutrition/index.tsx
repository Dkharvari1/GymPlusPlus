...
type Meal = { foodId: string; portions: number; type?: "breakfast" | "lunch" | "dinner" | "snack" };
...
export default function NutritionHome() {
  ...
  const [waterInput, setWaterInput] = useState("250");          // default ml
  const [meals, setMeals]       = useState<Meal[]>([]);
  const [catalog, setCatalog]   = useState<FoodCatalog>({});
  const [filter, setFilter]     = useState<Filter>("all");

  /* modal for creating arbitrary days */
  const [modalOpen, setModalOpen] = useState(false);
  const [pickerDate, setPickerDate] = useState(new Date());

  /* ── 1) Ensure TODAY exists, then load day ids ──────────────── */
  useEffect(() => {
    if (!uid) return;
    ...
    (async () => {
      ...
      if (!ids.includes(todayId)) {
        ...
        ids = [todayId, ...ids];          // put today at the top
      }

      setDays(ids);
      setDateId(todayId);                 // always land on today first
    })();
  }, [uid]);
  ...
  /* ── 3) realtime meals & water for the selected day ─────────── */
  useEffect(() => {
  ...
  /* ── helper: parse water amount from input ──────────────────── */
  const parseAmount = () => {
  ...
  const changeWater = async (delta: number) => {
    ...
    await updateDoc(doc(db, "users", uid, "days", dateId), { water: increment(delta) });
  };
      ...
      <LinearGradient colors={["#7c3aed", "#4f46e5", "#312e81"]} style={styles.bg}>
        <ScrollView contentContainerStyle={{ paddingBottom: 120 }}>
          ...
          <View style={styles.headerRow}>
            ...
            <TouchableOpacity style={styles.iconBtn} onPress={() => setModalOpen(true)}>
              <MaterialCommunityIcons name="calendar-plus" size={20} color="#fff" />
            </TouchableOpacity>
            {dateId && (
              <TouchableOpacity style={styles.iconBtn} onPress={confirmDeleteDay}>
                <MaterialCommunityIcons name="trash-can" size={20} color="#fca5a5" />
              </TouchableOpacity>
            )}
          </View>
          ...
          <View style={styles.waterRow}>
            ...
            />
            <Pressable style={styles.waterAdd} onPress={addWater} disabled={!dateId}>
              <MaterialCommunityIcons name="plus" size={16} color="#fff" />
            </Pressable>
            <Pressable style={styles.waterSub} onPress={removeWater} disabled={!dateId}>
          ...
          {filteredMeals.length > 0 && (
            <>
              ...
              {MEAL_ORDER.filter(t => (filter === "all" ? true : t === filter)).map(type => {
...
const styles = StyleSheet.create({
  ...
  iconBtn:{ padding:8,borderRadius:16,backgroundColor:"rgba(255,255,255,0.15)",marginLeft:8 },
  pillBar:{ flexDirection:"row",flexWrap:"wrap",gap:8,marginBottom:18 },
  pill:{ paddingHorizontal:14,paddingVertical:6,borderRadius:16,backgroundColor:"rgba(255,255,255,0.12)" },
  ...
  waterRow:{ flexDirection:"row",alignItems:"center",gap:8,alignSelf:"flex-end",marginTop:10,marginBottom:6 },
  waterInput:{ minWidth:60,paddingVertical:6,paddingHorizontal:10,borderRadius:12,backgroundColor:"rgba(255,255,255,0.1)",color:"#fff",textAlign:"center",fontSize:13 },
  waterAdd:{ backgroundColor:"#22c55e",padding:8,borderRadius:14 },
  waterSub:{ backgroundColor:"#ef4444",padding:8,borderRadius:14 },
  btn:{ marginTop:24,flexDirection:"row",alignItems:"center",justifyContent:"center",backgroundColor:"#4f46e5",borderRadius:24,paddingVertical:14 },
  ...
  mealRow:{ flexDirection:"row",alignItems:"center",gap:14,backgroundColor:"rgba(255,255,255,0.15)",borderRadius:18,padding:14,marginBottom:10 },
  mealName:{ color:"#fff",fontWeight:"600" },
  mealKcal:{ color:"#e5e7eb",fontSize:12 },
  modalBackdrop:{ flex:1,backgroundColor:"rgba(0,0,0,0.55)",justifyContent:"center",alignItems:"center",paddingHorizontal:32 },
  modalCard:{ width:"100%",borderRadius:22,padding:24,backgroundColor:"#1e1b4b" },
  modalTitle:{ fontSize:18,fontWeight:"700",color:"#fff",marginBottom:18,alignSelf:"center" },
  modalBtns:{ flexDirection:"row",justifyContent:"flex-end",marginTop:24 },
  modalCancel:{ paddingVertical:10,paddingHorizontal:18,borderRadius:14,marginRight:10,backgroundColor:"rgba(255,255,255,0.08)" },
  modalSave:{ paddingVertical:10,paddingHorizontal:18,borderRadius:14,backgroundColor:"#4f46e5" },
