...
import {
  View, Text, SectionList, Pressable, StyleSheet, ActivityIndicator,
  Modal, ScrollView, SafeAreaView, Platform, Alert
} from 'react-native';
import DateTimePicker from '@react-native-community/datetimepicker';
import { AntDesign, MaterialCommunityIcons, Feather } from '@expo/vector-icons';
import {
  collection, getDocs, query, orderBy, doc, setDoc, Timestamp,
  deleteDoc
...
function Chip({ label, active, onPress }:{
  ...
  return (
    <Pressable onPress={onPress} style={[styles.chip, active && { backgroundColor: purple }]}>
...
export default function WorkoutsHome() {
  ...
  /* ── day handling (mirror of Nutrition) ─────────────────────── */
  ...
  const sections = useMemo(() => {
    if (group === 'none') return [{ title:'ALL WORKOUTS', data: uniqByName(filtered) }];
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.dateRow}>
        <Text style={styles.h1}>{dateId ? `Workouts • ${dateId}` : 'No day selected'}</Text>

        {/* view log icon */}
        {dateId && (
          <Pressable
            style={styles.iconBtn}
            onPress={() => router.push({ pathname:'/workouts/day-log', params:{ day:dateId } })}
          >
      ...
      {/* sheet */}
      <Modal animationType="slide" visible={sheetOpen} onRequestClose={() => setSheetOpen(false)}>
...
const styles = StyleSheet.create({
  ...
  dateRow:{ flexDirection:'row', alignItems:'center', padding:16, backgroundColor:purple },
  h1:{ flex:1, color:'#fff', fontSize:20, fontWeight:'700' },
  iconBtn:{ padding:6, backgroundColor:'rgba(0,0,0,0.15)', borderRadius:14, marginLeft:6 },
  ...
  chip:{ borderWidth:1, borderColor:purple, borderRadius:18,
         paddingVertical:8, paddingHorizontal:18, flexGrow:1, alignItems:'center' },
  ...
  backdrop:{ flex:1, backgroundColor:'rgba(0,0,0,0.55)', justifyContent:'center',
             alignItems:'center', paddingHorizontal:32 },
  modal:{ width:'100%', borderRadius:22, padding:24, backgroundColor:'#1e1b4b' },
  modalTitle:{ fontSize:18, fontWeight:'700', color:'#fff', marginBottom:18, alignSelf:'center' },
  modalCancel:{ paddingVertical:10, paddingHorizontal:18, borderRadius:14,
                marginRight:10, backgroundColor:'rgba(255,255,255,0.08)' },
  modalSave:{ paddingVertical:10, paddingHorizontal:18, borderRadius:14, backgroundColor:purple },
