...
export default function LogExercise() {
  ...
  const submit = async () => {
    ...
    if (!snap.exists()) {
      // create the day doc only once, with empty arrays
      await setDoc(ref, { workouts: [], meals: [], water: 0, createdAt: new Date() });
    }
    ...
    /* arrayUnion prevents accidental overwrites & race conditions */
    await updateDoc(ref, { workouts: arrayUnion(entry) });
    router.back();
  };
        ...
        <View style={s.unitBar}>
          {(['kg', 'lb'] as const).map(u => (
            <Pressable
              ...
              onPress={() => setUnit(u)}
            >
              <Text style={[s.unitTxt, unit === u && s.unitTxtActive]}>
                {u.toUpperCase()}
              </Text>
            </Pressable>
          ))}
